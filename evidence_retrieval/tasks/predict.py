...
def predict(model, text_embedding, test_loader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="ĐANG TIẾN HÀNH LẤY KẾT QUẢ DỰ ĐOÁN"):
            ...
            #append res
            prediction["id"] = prediction["id"] + id#.detach().numpy().tolist()
            prediction["evidence"] = prediction["evidence"] + out_soft.argmax(dim=-1).cpu().detach().numpy().tolist()
...
def main(config):
    ...
    test_loader = getDataloader(test_dataset, config["predict"]["batch_size"])
